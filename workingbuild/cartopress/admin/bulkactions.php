	...
	class cartopress_bulkactions {
				...
				?>
					<script type="text/javascript">
						jQuery(document).ready(function() {
							...
							jQuery('<option>').val('cartopress_restore').text('<?php _e('Restore To CartoDB')?>').appendTo("select[name='action']");
							jQuery('<option>').val('cartopress_restore').text('<?php _e('Restore To CartoDB')?>').appendTo("select[name='action2']");
							//jQuery('div.media-toolbar-secondary').append('div class="blank">some text</div>');
						});
					</script>
				<?php
	    	}
		...
		function custom_bulk_admin_notices() {
			...
			if($pagenow == 'edit.php' && $post_type == 'post' && isset($_REQUEST['cartopress_deleted']) && (int) $_REQUEST['cartopress_deleted']) {
				$message = sprintf( _n( 'Post Deleted From CartoDB.', '%s posts deleted.', $_REQUEST['cartopress_deleted'] ), number_format_i18n( $_REQUEST['cartopress_deleted'] ) );
				echo "<div class=\"updated\"><p>{$message}</p></div>";
			}
			if($pagenow == 'edit.php' && $post_type == 'post' && isset($_REQUEST['cartopress_restored']) && (int) $_REQUEST['cartopress_restored']) {
				$message = sprintf( _n( 'Post Restored To CartoDB.', '%s posts restored.', $_REQUEST['cartopress_restored'] ), number_format_i18n( $_REQUEST['cartopress_restored'] ) );
				echo "<div class=\"updated\"><p>{$message}</p></div>";
			}
			if($pagenow == 'edit.php' && $post_type == 'page' && isset($_REQUEST['cartopress_deleted']) && (int) $_REQUEST['cartopress_deleted']) {
				$message = sprintf( _n( 'Page Deleted From CartoDB.', '%s pages deleted.', $_REQUEST['cartopress_deleted'] ), number_format_i18n( $_REQUEST['cartopress_deleted'] ) );
				echo "<div class=\"updated\"><p>{$message}</p></div>";
			}
			if($pagenow == 'edit.php' && $post_type == 'page' && isset($_REQUEST['cartopress_restored']) && (int) $_REQUEST['cartopress_restored']) {
				$message = sprintf( _n( 'Page Restored To CartoDB.', '%s pages restored.', $_REQUEST['cartopress_restored'] ), number_format_i18n( $_REQUEST['cartopress_restored'] ) );
				echo "<div class=\"updated\"><p>{$message}</p></div>";
			}
			if($pagenow == 'upload.php' && $post_type == 'attachment' && isset($_REQUEST['cartopress_deleted']) && (int) $_REQUEST['cartopress_deleted']) {
				$message = sprintf( _n( 'Attachment Deleted From CartoDB.', '%s attachments deleted.', $_REQUEST['cartopress_deleted'] ), number_format_i18n( $_REQUEST['cartopress_deleted'] ) );
				echo "<div class=\"updated\"><p>{$message}</p></div>";
			}
			if($pagenow == 'upload.php' && $post_type == 'attachment' && isset($_REQUEST['cartopress_restored']) && (int) $_REQUEST['cartopress_restored']) {
				$message = sprintf( _n( 'Attachment Restored To CartoDB.', '%s attachments restored.', $_REQUEST['cartopress_restored'] ), number_format_i18n( $_REQUEST['cartopress_restored'] ) );
				echo "<div class=\"updated\"><p>{$message}</p></div>";
			}
		}
